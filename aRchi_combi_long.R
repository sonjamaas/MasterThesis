...
# allowed_lengths <- c(3,4,6)  
# initial_allowed <- c(10,12,14,16,18)

# Define allowed segment lengths
first_segment_lengths <- c(18, 16, 14, 12, 10)  # in meters, descending order
other_segment_lengths <- c(3)             # in meters, descending order


# initialize segment stats table
segment_stats_final <- data.frame(Tree = numeric(), Segment = numeric(), Length = numeric(),
                                  Max_Deviation = numeric(), Class = character(), Start_Z = numeric(), End_Z = numeric(),
                                  Diameter = numeric(), Diameter_noBark = numeric(), Diameter_class = character(),
                                  FirstBranch_height = numeric(), radial_variance = numeric(), volume = numeric(), Branch_Count = numeric())
...
for (l in 1:length(valid_trees)){
  ...
  segment_stats <- data.frame(Tree = numeric(), Segment = numeric(), Length = numeric(),
                              Max_Deviation = numeric(), Class = character(),
                              Start_Z = numeric(), End_Z = numeric(),
                              Diameter = numeric(), Diameter_noBark = numeric(),
                              Diameter_class = character(), FirstBranch_height = numeric(),
                              radial_variance = numeric(), volume = numeric(),
                              # Has_Branch = logical(),
  ...
  archi <- skeletonize_pc(archi, D = 1, cl_dist = 0.3, max_d = 1.5, progressive = TRUE)
  ...
  for (m in seq_len(nrow(main_stem))) {
    ...
    mask <- points_in_cylinder(pc, start, end, max_radius = radius+0.1, min_radius = 0, buffer = 0)
  ...
  signed_curvature <- deviations[,1] * axis_vector[2] - deviations[,2] * axis_vector[1]  # Signed curvature in 2D
  ...
  segment_count <- 0
  current_idx <- 1
  max_idx <- nrow(stem_sorted)
  
  while (current_idx < max_idx) {
    segment_count <- segment_count + 1
    start_idx <- current_idx
    start_height <- stem_sorted[start_idx, 3]
    remaining_height <- stem_sorted[max_idx, 3] - start_height
    
    if (segment_count == 1) {
      # First segment: choose the longest possible between 10 and 18m
      possible_lengths <- first_segment_lengths[first_segment_lengths <= remaining_height]
      if (length(possible_lengths) == 0) {
        segment_length <- remaining_height
      } else {
        segment_length <- max(possible_lengths)
      }
    } else {
      # Subsequent segments: choose 6, 4, or 3m
      possible_lengths <- other_segment_lengths[other_segment_lengths <= remaining_height]
      if (length(possible_lengths) == 0) {
        segment_length <- remaining_height
      } else {
        segment_length <- max(possible_lengths)
    ...
    # Find the index where this segment should end
    end_height <- start_height + segment_length
    # Find the index of the point closest to end_height but not exceeding max_idx
    end_idx <- which(stem_sorted[,3] >= end_height)[1]
    if (is.na(end_idx) || end_idx > max_idx) {
      end_idx <- max_idx
    }
  ...
  merged_segment_indices <- final_segments[,1]
  ...
  for (o in 1:(length(merged_segment_indices) - 1)) {
    ...
    segment_i <- stem_sorted[merged_segment_indices[o]: merged_segment_indices[o + 1], ]
    
    if(nrow(segment_i) < 150){next}
    ...
    for (h in slice_heights) {
      slice <- subset(segment_i, segment_i[, 3] >= h - 0.1 & segment_i[, 3] < h + 0.1)
      if (nrow(slice) > 3) {
        # Ensure enough points for fitting
        circle <- fitSS(slice)
        center <- cbind(circle$par[[1]], circle$par[[2]])
        centerline <- rbind(centerline, data.frame(Z = h, Xc = center[1, 1], Yc = center[1, 2]))
      }
    }
    
    # Create LAS data with RGB attributes for segment
    las_data <- data.frame(X = segment_i[, 1], Y = segment_i[, 2], Z = segment_i[, 3])
    
    # Create LAS data with RGB attributes for center line
    las_data2 <- data.frame(X = centerline$Xc, Y = centerline$Yc, Z = centerline$Z)
    ...
    # Define slice positions (heights)
    z_start <- stem_sorted[segment_start, 3]
    z_end <- stem_sorted[segment_end, 3]
    z_mid <- (z_start + z_end) / 2
    z_q1 <- z_start + 0.25 * (z_end - z_start)
    z_q3 <- z_start + 0.75 * (z_end - z_start)
    
    slice_positions <- c(z_start, z_q1, z_mid, z_q3, z_end)
    diameters <- numeric(length(slice_positions))
    
    # calculate segment length
    segment_length <- abs(stem_sorted[segment_end, 3] - stem_sorted[segment_start, 3])
    
    repeat {
      start_slice <- subset(segment_i, segment_i[, 3] >= stem_sorted[segment_start, 3] & segment_i[, 3] < (stem_sorted[segment_start, 3] + slice_size))
      # Check if enough points are selected or window is too large
      if (nrow(start_slice) >= min_points || slice_size >= max_window) {
        break
      }
    ...
    for (k in seq_along(slice_positions)) {
      z_pos <- slice_positions[k]
      slice <- subset(segment_i, abs(segment_i[, 3] - z_pos) < slice_size)
      if (nrow(slice) >= min_points) {
        circle <- fitSS(slice)
        diameters[k] <- circle[[1]][3] * 2
      } else {
        diameters[k] <- NA  # Not enough points, will be ignored in mean
      }
    }
    ...
    repeat {
      mid_z <- (stem_sorted[segment_end, 3] + stem_sorted[segment_start, 3]) / 2
      mid_slice <- subset(segment_i, segment_i[, 3] >= (mid_z - slice_size) & segment_i[, 3] < (mid_z + slice_size))

      # Check if enough points are selected or window is too large
      if (nrow(mid_slice) >= min_points || slice_size >= max_window) {
        break
      }
    ...
    # Remove NAs and calculate mean diameter
    diameters <- c(diameters, diameter, diameter_start)
    diameters <- diameters[!is.na(diameters) & diameters < 1]
    
    # if (is.nan(mean(diameters)) || mean(diameters) >= 1){next} else {}
    ...
    # Use mean_diameter for volume calculation
    if (mean_diameter > 1 || is.nan(mean(diameters))) {
      next
    } else if (mean_diameter <= 0.41){
      diameter_noBark <- mean_diameter - 0.01} else{
      diameter_noBark <- mean_diameter - 0.02
    }
    ...
    radial_variance <- compute_radial_variance(mid_slice, mid_circle[[1]][1], mid_circle[[1]][2])
    ...
    branches <- sum(fork_heights[c(3:length(fork_heights))] >= stem_sorted[segment_start, 3] & fork_heights[c(3:length(fork_heights))] < stem_sorted[segment_end, 3])
    
    if(is.na(branches)){branches <- 0}
    
    # rvr classification
    class <- rvr_classification(segment_length, max_curvature, diameter_noBark, branches)
    ...
    segment_stats <- rbind(segment_stats,
                           data.frame(Tree = tree_i, Segment = o, Length = segment_length,
                             Max_Deviation = max_curvature, Class = class, Start_Z = stem_sorted[segment_start, 3],
                             End_Z = stem_sorted[segment_end, 3], Diameter = mean_diameter, Diameter_noBark = diameter_noBark,
                             Diameter_class = diameter_class, radial_variance = round(radial_variance, digits = 10),
                             volume = fm, Branch_Count = branches))
  } 
  ...
  if(nrow(segment_stats)>1){
  for (i in 2:nrow(segment_stats)) {
    plausible[i] <- segment_stats$Diameter_noBark[i] < segment_stats$Diameter_noBark[last_plausible] * 1.2 & segment_stats$Diameter_noBark < 1
    if(plausible[i]){
      last_plausible <- i
    }
  }}
  ...
  for (i in length(plausible):1) {  # Iterate backwards!
    if(plausible[i]) {
      writeLAS(las_segments_list[[1]], paste("tree_", tree_i, "_segment_", 1, ".las"))
      writeLAS(las_centerlines_list[[i]], paste("tree_", tree_i, "_centerline_", i, ".las"))
      # plot( mid_slices_list[[i]], asp = 1,
  ...
  cat("Total Stem Volume: ", round(sum(segment_stats_final$volume),  digits = 3), "m³", "\n" , 
      "Volume Quality A: ", round(sum(subset(segment_stats_final, segment_stats_final$Class == "A")$volume), digits = 3), "m³", "\n",
      "Volume Quality B: ", round(sum(subset(segment_stats_final, segment_stats_final$Class == "B")$volume),  digits = 3), "m³", "\n",
      "Volume Quality C: ", round(sum(subset(segment_stats_final, segment_stats_final$Class == "C")$volume),  digits = 3), "m³", "\n",
      "Volume Quality D: ", round(sum(subset(segment_stats_final, segment_stats_final$Class == "D")$volume),  digits = 3), "m³")
  ...
  ### 3.3 skeletonize aRchi object and add radius
  archi_ForCrown <- skeletonize_pc(archi_ForCrown, D = 0.5, cl_dist = 0.3, max_d = 1.5, progressive = TRUE)
  # plot(archi_ForCrown)
  ### 3.4 Add radius to aRchi object  
  archi_ForCrown <- add_radius(archi_ForCrown, sec_length = 0.5, by_axis = TRUE, method = "median")
